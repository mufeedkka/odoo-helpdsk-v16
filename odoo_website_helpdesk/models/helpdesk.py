...
class HelpDeskTicket(models.Model):
    ...
    team_head = fields.Many2one('res.users', string='Team Leader',
                                compute='_compute_team_head', store=True)
    ...
    product_module = fields.Selection([])
    ...
    def team_leader_domain(self):
        ...
        for rec in self.team_id.member_ids:
            li.append(rec.id)
        print('li', li)
        # fetch_team_member = self.team_id.team_lead_id.id
    ...
    # show_subcategory = fields.Boolean(default=_default_show_subcategory,
    #                                   compute='_compute_show_sub_category')
    customer_rating = fields.Selection(PRIORITIES1, default='0', readonly=True)
    ...
    #
    # def _compute_show_sub_category(self):
    #     show_subcategory = self._default_show_subcategory()
    #     for rec in self:
    #         rec.show_subcategory = show_subcategory
        ...
        if auto_close:
            ...
            for rec in records:
                ...
                if days >= int(no_of_days):
                    close_stage_id = self.env['ticket.stage'].search([('closing_stage', '=', True)])
                    if close_stage_id:
...
class StageTicket(models.Model):
    ...
    template_id = fields.Many2one('mail.template', domain="[('model', '=', 'help.ticket')]")
    ...
    def unlink(self):
        for rec in self:
            ...
            lowest_sequence = tickets.filtered(lambda x: x.sequence == min(sequence))
            if self.name=="Draft":
