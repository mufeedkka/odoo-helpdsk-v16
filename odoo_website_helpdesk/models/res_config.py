...
class Menu(models.Model):
    ...
    def _compute_visible(self):
        ...
        show_menu_header = self.env['ir.config_parameter'].sudo().get_param('odoo_website_helpdesk.helpdesk_menu_show')
        for menu in self:
...
class Helpdesk(models.TransientModel):
    ...
    # show_subcategory = fields.Boolean(string="Sub Category",
    #                                   config_parameter='odoo_website_helpdesk.show_subcategory',
    #                                   implied_group='odoo_website_helpdesk.group_show_subcategory')
    ...
    def show_category_subcategory(self):
        if self.show_category:
            group_cat = self.env.ref('odoo_website_helpdesk.group_show_category')
            group_cat.write({
        ...
        # if self.show_subcategory:
        #     group_sub_cat = self.env.ref('odoo_website_helpdesk.group_show_subcategory')
        #     group_sub_cat.write({
        #         'users': [(4, self.env.user.id)]
        #     })
        # else:
